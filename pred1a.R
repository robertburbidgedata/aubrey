...
for(groupi in groups){
  ...
  x<-cbind(1,as.matrix(train1[,newpredvars]))
  y<-as.numeric(as.character(train1$cat))
  xv<-cbind(1,as.matrix(val1[,newpredvars]))
  ...
  for(lambdai in 1:nlambda){
    ...
    loss_lambda[[groupi]][lambdai]<-attr(ordinalRegressionLoss(xv,yv,C=cost,impl="loglin")(nrbm.w),"lvalue")
  }
  ...
  x<-cbind(1,as.matrix(train[,newpredvars]))
  y<-as.numeric(as.character(train$cat))
  xt<-cbind(1,as.matrix(test[,newpredvars]))
  ...
  pred<-factor(apply(predict(clm1,df[,c("yhat","exp1","exp2","exp3")])$fit,1,which.max),levels=levels(ystar))
  ...
  # non-dld preds
  dat2<-dat[!idx_dld,]
  x2<-cbind(1,as.matrix(dat2[,newpredvars]))
...
save(list=ls(all.names=TRUE),file="/wps_scratch/usr_scratch/burbidge/Aubrey/finalworkspace1a.RData"
     ,envir=.GlobalEnv)
...
write.csv(outpreddata,row.names=F
          ,file="/wps_scratch/usr_scratch/burbidge/Aubrey/finalpreds1a.csv")
