# gpm_eda2.R
# add AMENITY and REVIEW fields from HCP_JOIN_15
rm(list=ls())
...
apply(is.na(HCP_JOIN_15),2,sum)

# check FSA representation
length(unique(demographics$ONEKEY_ID))
length(unique(HCP_JOIN_15$ACCOUNT_ONEKEY_ID))
tmp<-merge(demographics[,c("ONEKEY_ID","FSA")]
           ,data.frame(ACCOUNT_ONEKEY_ID=HCP_JOIN_15[,"ACCOUNT_ONEKEY_ID"]),
           by.x="ONEKEY_ID",by.y="ACCOUNT_ONEKEY_ID")
tmp<-droplevels(tmp)
dim(tmp)
length(levels(tmp$FSA))
# 77 FSA not represented on HCP_JOIN_15
setdiff(levels(demographics$FSA),levels(tmp$FSA))
...
ctsvars_HCP15<-setdiff(names(HCP_JOIN_15),c(ohevars,"ACCOUNT_ONEKEY_ID"))
...
length(levels(HCP_JOIN_15$FSA))
apply(is.na(HCP_JOIN_15[,ctsvars_HCP15]),2,sum)
dim(HCP_JOIN_15)
...
# missing values
for (ctsvar in c(ctsvars_HCP15,ohevars)){
  HCP_JOIN_15_fsa[is.na(HCP_JOIN_15_fsa[,ctsvar]),ctsvar]<-
    mean(HCP_JOIN_15_fsa[!is.na(HCP_JOIN_15_fsa[,ctsvar]),ctsvar])
}
apply(is.na(HCP_JOIN_15_fsa[,c(ctsvars_HCP15,ohevars)]),2,sum)
HCP_JOIN_15_fsa<-droplevels(HCP_JOIN_15_fsa)
length(levels(HCP_JOIN_15_fsa$FSA))
...
dim(demographics_fsa)

# merge in new features
demographics_fsa<-merge(demographics_fsa,HCP_JOIN_15_fsa,"FSA")
...
for (ctsvar in ctsvars_HCP15){
  x_fsa[,ctsvar]<-(x_fsa[,ctsvar]-min(x_fsa[,ctsvar]))/
    (max(x_fsa[,ctsvar])-min(x_fsa[,ctsvar]))
}
